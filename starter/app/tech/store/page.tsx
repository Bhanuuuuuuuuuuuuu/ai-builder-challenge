...
export default function TechStorePage() {
  ...
  async function handleLocationScan(value: string) {
    ...
    try {
      ...
      setMessage(`Asset ${assetTag} stored at ${value}.`);
      setStep("done");
    } catch (err) {
      setMessage(
        err instanceof Error
          ? `Could not store asset: ${err.message}`
    ...
    <div className="max-w-xl mx-auto p-4 space-y-6">
      <div>
        ...
        <p className="text-gray-600 mt-2">
          Scan an asset tag, then scan the storage location.
        </p>
      </div>
      ...
      <button
        ...
        disabled={busy}
      >
        {step === "done" ? "Store another asset" : "Reset"}
      </button>
    </div>
  );
}